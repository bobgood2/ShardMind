...
def load_embeddings_from_directory(directory, expected_embedding_dim):
    ...
    for file_name in os.listdir(directory):
        if file_name.endswith('.npy'):
            ...
            try:
                with open(config.raw_path(file_path), 'r') as file:
...
def raw_path(file_path):
    return config.change_dir_and_ext(file_path,config.CALENDAR_RAW_DIR, '.json') 
...
embeddings, filenames, ages = load_embeddings_from_directory(config.CALENDAR_EMBEDDINGS_DIR, 384)
